...
if (isset($_GET['action']) && ($_GET['action'] == 'add' || $_GET['action'] == 'edit')) {
    if (isset($_POST['name']) && !empty($_POST['name']) && isset($_POST['price']) && !empty($_POST['price'])) {
        ...
        if (empty($_POST['name'])) {
            ...
            $warning_msg[] = "Bạn phải nhập phân loại sản phẩm!";
        ...
        if (!isset($error)) {
            if ($_GET['action'] == 'edit' && !empty($_GET['id'])) { //Cập nhật lại sản phẩm
                ...
                if ($typeUD != 1 && $typeUD != 2 && $typeUD != 3 && $typeUD != 4 && $typeUD != 5 && $typeUD != 6) {
                    $warning_msg[] = 'Bạn không thay đổi gì!';
                } else { //Cập nhật sản phẩm
                    $result = mysqli_query($conn, "UPDATE `menu` SET `name` = '" . $_POST['name'] . "',`image` =  '" . $image . "',
                    `price` = " . str_replace('.', '', $_POST['price']) . ", `id_pro` = " . $typeUD . ", `content` = '" . $_POST['content'] . "' WHERE `menu`.`id` = " . $_GET['id']);
